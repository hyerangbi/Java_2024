    ...
    <div class="content">
        ...
        <script>
            $(function(){
                ...
                idInput.onkeyup = function(ev){ //키가 입력될 때마다 호출 ( 체크 )
                    clearTimeout(eventFlag); // 아직 실행되지 않은 ajax 요청을 취소

                    const str = ev.target.value;

                    if(str.trim().length >= 5){ // 5글자보다 크거나 같을 때 요청을 보낸다.
                        eventFlag = setTimeout(function(){ // 1초 뒤에 값을 확인해서 ajax 요청
                                    ...
                                    $.ajax({
                                        ...
                                        success : function(result){
                                            const checkRerult = document.querySelector("#checkResult");
                                            checkResult.style.display = "block";

                                            if(result === "NNNNN"){
                                                checkResult.style.color = "red";
                                                checkRerult.innerText = "이미 사용중인 아이디입니다."
