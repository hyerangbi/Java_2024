...
public class Run {
	/*
	  	...
	  	- 컴파일에러 : 소스코드 문법 상의 오류 → 빨간 줄로 애초에 오류를 알려준다.(개발자 실수) → 발견과 해결이 쉽다.
	  	- 런타임에러 : 코드 상으로 문제가 없지만 프로그램 실행도중 발생하는 에러 (사용자의 실수 또는 개발자가 미처 처리하지 않은 기능)
	  	- 논리에러 : 문법적으로도 문제 없고 실행했을 때도 문제는 없지만 프로그램의 의도와 맞지 않는 동작이나 결과가 나타나는 에러 (찾기가 힘들다)
	  	
	  	컴파일에러, 런타임에러, 논리에거 같은 개발자가 예측가능하며 수정할 수 있는 에러들을 가지고 작업
...
/*		
		...
		try {
			...
			e.printStackTrace();
		} finally { //예외가 발생하건 안하건 try - catch 종류 후 마지막에 실행하는 코드
			sc.close();
		}
*/		
