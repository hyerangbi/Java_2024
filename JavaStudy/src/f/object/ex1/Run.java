...
public class Run {
	...
	/*
	 	...
	 	4. 구현하고자 하는 프로그램상의 필요한 객체들을 만들기 위해서(즉, 생성하기 위해서)
	 		: 클래스라는 설계도가 먼저 필요하다. (클래스? 각 객체들의 속성(데이터들을) 담아낼 그릇같은 존재)
	 	...
	 	5. 추상화 과정
		 	...
		 	ex) 학생 관리 프로그램
		 	1) 학생관련 객체(ㅁㅁㅁ훈련생, ㅇㅇㅇ훈련생, △△△훈련생 등등 .. )
		 	2) 공통적인 속성 및 기능 (이름, 나이, 주소, 전화번호, 키, 몸무게, 머리색, 발사이즈, 달리기속도, 먹는다, 잔다, 화장실간다, 공부한다, 휴식한다)
	 	...
	 	6. 추상화한 객체를 프로그램에 적용시키기
	 		1. "변수"만으로 프로그래밍을 한다.
	 			...
	 			A 라는 훈련생 객체 하나 만들기 -> String name1 = "A"; int age = 20; double height = 185.0;
	 			B 라는 훈련생 객체 하나 만들기 -> String name2 = "A"; int age2 = 19; double height2 = 193;
	 			
	 		2. 배열로 프로그래밍 한다면
	 			배열 : 하나의 자료형으로 여러 개의 값들을 보관할 수 있음
	 			이름을 보관하는 배열 String[] name = {"A", "B" ...}
	 			나이를 보고나하는 배열 int[] age = {20, 19 ...} 
	 			
	 		3. 그래서 나온 개념 → 구조체
	 		구조체 : 여러 개의 자료형의 여러 개의 값들을 보관할 수 있다.
	 		String 값도 보관하고 int 값도 보관하고 동시에 double값도 함께 보관하는 자료형을 직접 만든다고 생각 → 나만의 자료형
	 		Struct human{
	 			String name;
	 			int age;
	 			double height;
	 	...
	 	◾ 캡슐화 : 추상화를 통해서 정의된 속성들과 기능들을 하나로 묶어서 관리하는 기법
	 			 클래스에서 가장 중요한 목적인 "데이터 접근제한(정보은닉)"을 원칙으로
	 			 외부로부터 "데이터접근을 막고"
	 			 대신에 "데이터를 간접적으로 처리(값을 변경하거나, 가져오는 것등)"할 메소소들을 클래스 내부에 작성하여 관리하는 것
