		...
		if(JakartaServletFileUpload.isMultipartContent(request)) {
			...
			for(FileItem item : formItems) {
				...
				if(item.isFormField()) { // 일반파라미터
					...
					case "originFileNo":
						originFileNo = item.getString(Charset.forName("uft-8"));
						break;
					}
				...
				// 새로운 첨부파일 o, 기존첨부파일 x		➡	b, refBoardNo -> board update, attachment insert
				
				if(result > 0) { // 성공 -> 게시글 상세페이지(jps/detail.bo?bno=게시글번호)
					request.getSession().setAttribute("alartMsg", "일반 게시글 수정 성공");
