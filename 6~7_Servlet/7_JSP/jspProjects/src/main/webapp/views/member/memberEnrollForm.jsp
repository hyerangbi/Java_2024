...
<body>
    ...
    <script>
        ...
        function idCheck(){
            // 중복확인버튼 클릭 시 사용자가 입력한 아이디 값을 서버에 보내서 조회 요청 -> 응답받기
            // 1) 사용불가 -> alert메세지 출력(이미 존재하는 아이디 입니다.)
            // 2) 사용가능 -> 진짜 사용할 것인가 ? -> ok :더 이상 아이디 수정 할 수 없음
            //                                      no : 다시 입력하도록 유도하기

            const idInput = querySelector("input[name=userId]");
            ...
            $.ajax({
                ...
                success : function(result){
                    if(result === "NNNNY"){
                        ...
                        const submitBtn = querySelector("#enroll-form input[name=userId]");
