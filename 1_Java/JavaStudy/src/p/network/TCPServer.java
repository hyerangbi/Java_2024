...
public class TCPServer {
	public static void main(String[] agrs) {
		...
		// ⑴ 포트번호 지정 (서버측에서 몇번 포트로 통로를 열것인가?)
		int port = 3000;
		
		try {
			// ⑵ ServerSocket 객체생성시 포트와 결합(Bind)
			ServerSocket server = new ServerSocket(port);
			
			System.out.println("클라이언트 요청을 기다리고 있습니다.");
			
			//⑶ 요청을 기다렸다 요청이 오면 → 요청 수락 후 클라이언트와 통신할 수 있는 서버 쪽 소켓객체 생성
			...
			//입력용 스트림 (클라이언트로부터 전달된 값을 한 줄 단위로 읽어드리기 위해서 입력용 스트림 사용)
