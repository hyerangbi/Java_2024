...
-- 7. EMPLOYEE테이블에서 SAL_LEVEL이 S1인 사원의 이름, 월급, 고용일, 연락처 조회 (S1 => 6000000 ~ 10000000 급여가 되는 사람)
SELECT EMP_NAME, SALARY, HIRE_DATE, PHONE FROM EMPLOYEE WHERE SALARY BETWEEN 6000000 AND 10000000;

-- 8. EMPLOYEE테이블에서 실수령액(6번 참고)이 5천만원 이상인 사원의 이름, 월급, 실수령액, 고용일 조회
SELECT 
EMP_NAME, SALARY,
((SALARY + (SALARY * NVL(BONUS, 0))) * 12)-(SALARY*12*0.03) AS "실수령액" ,HIRE_DATE 
FROM EMPLOYEE
;
...
SELECT EMP_NAME "사원 명",
      ...
      SUBSTR(EMP_NO, 3, 4) AS "생월",
      SUBSTR(EMP_NO, 5, 6) AS "생일"
FROM EMPLOYEE;
...
SELECT EMP_NAME, TO_CHAR(SALARY, 'L99,999,999') 급여 FROM EMPLOYEE;
...
SELECT EMP_NAME, DEPT_CODE,
       ...
       SUBSTR(EMP_NO, 3, 4) || '월',
       SUBSTR(EMP_NO, 5, 6) || '일',
       -- EXTRACT (YEAR FROM SYSDATE) - EXTRACT (YEAR FROM TO_DATE(SUBSTR(EMP_NO,1,2),'YYYY'))
...
SELECT EMP_ID, EMP_NAME, DEPT_CODE, 
       DECODE(DEPT_CODE, 'D5','총무부','D6', '기획부','D9','영업부') FROM EMPLOYEE 
WHERE DEPT_CODE IN('D5', 'D6', 'D9')
ORDER BY DEPT_CODE;

-- 23. EMPLOYEE테이블에서 사번이 201번인 사원명, 주민번호 앞자리, 주민번호 뒷자리, 
--  주민번호 앞자리와 뒷자리의 합 조회
SELECT EMP_NAME, SUBSTR(EMP_NO, 1, 6)AS 앞자리,
