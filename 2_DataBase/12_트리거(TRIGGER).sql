/*
    ...
    EX)
    회원 탈퇴시 기존의 회원테이블에 데이터 DELETE 한 후 곧바로 탈퇴한 회원들만 따로 보관하는 테이블에
    자동으로 INSERT 시켜야한다.
    신고횟수가 일정 수를 넘었을 때 묵시적으로 해당 회원을 블랙리스트로 처리하게 되게끔
    입출고에 대한 데이터 기록(INSERT)될 때마다 해당 상품에 대한 재고수량을 매번 수정(UPDATE)해야한다.
    
    * 트리거의 종류
    - SQL문의 실행시기에 따른 분류
    - BEFOR TRIGGER : 내가 지정한 테이블에 이벤트가 발생되기 전에 트리거 실행
    - AFTER TRIGGER : 내가 지정한 테이블에 이벤트가 발생된 후 트리거 실행
    
    - SQL문에 의해 영향을 받는 각 행에 따른 종류
    - 문장트리거 : 이벤트가 발생한 SQL 문에 대해 딱 한 번만 트리거 실행
    - 행트리거 : 해당 SQL문 실행할 때마다 매번 트리거 실행
                (FOR EACH ROW옵션 기술해야한다.)
                > : OLD  - BEFORE UPDATE (수정 전 자료), BEFOR DELETE (삭제 전 자료)
                > : NEW  - ATER INSERT (추가 전 자료), AFTER UPDATE (수정 후 자료)
...
VALUES(SEQ_EID.NEXTVAL, '새우튀김', '111211-1111111', 'D7', 'J7', SYSDATE);
...
-- 상품 번호를 중복 안되게끔 매번 새로운 번호를 발생시키는 시퀀스 생성
...
-- TB_PRODUCT 테이블에 매번 자동응로 재고수량을 UPDATE하게끔 트리거 작성

/*
    - 상품이 입고된 경우 -> 해당상품을 찾아서 재고수량 증가 UPDATE
    UPDATE TB_PRODUCT 
