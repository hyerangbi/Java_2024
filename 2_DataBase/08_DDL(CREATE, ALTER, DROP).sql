/*
    ...
    *변경할내용 
    1) 컬럼 추가/수정/삭제
    2) 제약조건 추가/삭제 -> 수정은 불가 (삭제 후 추가 해야한다.)
    3) 컬럼명/제약조건명/테이블명 변경    
*/

-- 1) 컬럼 추가/수정/삭제
-- DEPT_TABLE 에 CNAME 컬럼 추가
...
-- DEFAULT 값 수정 : MODIFY 컬럼명 DEFAULT 변경할 기본 값
...
-- 다중변경가능  (아래처럼 한 번에 쓸 수 있다.)
ALTER TABLE DEPT_TABLE 
         MODIFY DEPT_TITLE VARCHAR2(40)
         MODIFY LNAME DEFAULT '미국';

-- 1_3) 컬럼 삭제( DROP COLUMN ) 구조를 없애거나 바꿀 때 DROP 사용 /데이터 관련해서 삭제할 때는 DELETE 
...
ALTER TABLE DEPT_COPY DROP COLUMN LOCATION_ID; -- 테이블은 최소 하나 이상의 컬럼이 필요하다.
...
-- 2_2) 제약조건 삭제 : DROP CONSTRAINT 제약조건명 / NOT NULL( 은 삭제안됨 )
...
/*
     삭제가 안되는 것 
   어딘가 참조 되고있는 부모테이블은 함부로 삭제가 되지않는다.
...
--=============================================================================
-- 3) 컬럼명 / 제약조건명 / 테이블명 변경
-- 3_1) 컬러명 변경 : RENAME COLUMN 기존컬럼명 TO 바꿀 컬럼명
