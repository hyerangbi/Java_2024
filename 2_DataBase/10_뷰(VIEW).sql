/*
  ...
  (자주 사용하는 SELECT문을 저장해주면 긴 SELECT문을 다시 기술할 필요없이 사용할 수 있다.)
  임시테이블 같은 존재 (실제 데이터가 담겨있는건 아니다 -> 논리테이블)
*/
...
-- GRANT CREATE VIEW TO KH; // 관리자계정으로 변환하여 권한 추가
DROP VIEW VW_VIEW;

SELECT * FROM VW_EMPLOYEE;

--실제 실행 되는 것은 아래와 같이 서브쿼리로 실행된다고 볼 수 있다.   //인라인뷰
SELECT *
...
-- CREATE OR REPLACE 를 사용하면 VIEW가 없을 때 생성. 이미 존재한다면 수정할 수 있다.
...
/*
    * 뷰 퀄럼에 별칭 부여
    서브쿼리에 SELECT 절에 함수식이나 산술연산식이 기술되어있다면 반드시 별칭을 부여해야한다.
*/
...
/*
    * DML 명령어로 조작이 불가능한 경우가 많다
    ...
    2) 뷰에 정의되어 있지 않은 컬럼 중에 베이스테이블 상에 NOT NULL 제약 조건이 지정되어 있는 경우    
    3) 산술연산식이나 함수식을 사용한 경우
    4) 그룹함수나 GROUP BY절을 포함한 경우
    5) DISITINST 구문이 포함된 경우
    6) JOIN을 이용해서 여러 테이블을 연결시켜놓은 경우
    
    대부분 뷰는 조회를 목적으로 생성한다. 그냥 뷰를 통한 DML은 안 쓰는게 좋다.    
*/
...
/*
    ...
    [상세표현식]
    CREATE [OR REPLACE] [FORCE | NOFORCE] VIEW 뷰 명
    AS 서브쿼리
    [WITH CHECK OPTION]
    [WITH READ ONLY];
    ...
    2) FORCE | NOFORCE
        ...
        > NOFORCE : 서브쿼리에 기술된 테이블이 존재하는 테이블이여만한다. (기본값)
    3) WITH CHECK OPTION : DML시 서브쿼리에 기술된 조건에 부합한 값으로 DML이 가능하도록
    4) WITH READ ONLY : 부에 대해서 조회만 가능하도록
*/
