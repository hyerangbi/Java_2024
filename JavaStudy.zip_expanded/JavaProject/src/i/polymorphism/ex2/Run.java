...
public class Run {
	public static void main(String[] agrs) {
		...
		/*
		 ...
		 다형성
		 : 부모타입으로부터 파생된 여러가지 타입의 자식객체들을
		 부모클래스 타입하나로 다룰 수 있는 기술
		...
		sonataArr[0] = new Sonata("빨간색", "가솔린", 2020);
		Avante[] avanteArr = new Avante[5];
		avanteArr[0] = new Avante("검은색", "디젤", 2022);
		
		//다향성 사용 후
		Car[] carArr = new Car[10];
		carArr[0] = new Sonata("흰색", "가스", 2023);
		carArr[1] = new Avante("검은색", "디젤", 2022);
		carArr[2] = new Sonata("흰색", "가스", 2023);
		carArr[3] = new Avante("검은색", "디젤", 2022);
		
		
		for(int i=0; i<carArr.length; i++) {
			//해당 참조변수가 실제 Sonata객체를 참조하고 있는가 ?
