...
public class B_String {
	public void method01() {
		...
		// String 클래스에 toString메소드는 이미 오버라이딩 되어있다.
		
		//주소 값을 비교하기 때문에 false가 나온다
		System.out.println(str1 == str2);
		
		//String클래스에서 equals 메소드는 이미 오버라이딩이 되어있다.
		//(주소 값이 아닌 값 비교)
		System.out.println(str1.equals(str2));
		
		//String클래스에서 hashCode메소드 이미 오버라이딩이 되어있음
		//주소 값이 아닌 문자열을 가지고 만들도록
		...
		// 새로운 곳을 참조하도록 됨(새로운 주소 값 부여받음 == 주소 값 변경)
	...
	public void method02() {
		...
		// 1. 문자열.charAr(int index) : char
		//	  문자열에 전달받은 index 위치에 문자만을 추출해서 리턴
		...
		//    문자열과 전달된 또 다른 문자열을 가지고 주소 값 비교하기
		...
		System.out.println(str1.substring(6, 9)); //6부터 8까지
	...
	public void method03() {
		...
		*/
		...
		/* 
		 System.out.println(stn.nextToken());
		◾ 더 이상 토큰이 남아있지 않을 때 다음 토큰을 가져오려고 하면 예외발생 'ㅁ'/				
		*/
		...
		//◾ hasMoreElements → 가져올 토큰이 남아있는지 확인해주는 메소드 남아있다면 true 반환 없다면 false 반환 
